...
test('GET /this/page/is/hidden/behind/an/incredibly/high/paywall/that/could/only/be/unlocked/by/sending/1btc-to-us', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    .createScan({
      tests: ['unvalidated_redirect', 'csrf', 'excessive_data_exposure', 'xss', 'sqli', 'nosql', 'osi', 'ssrf', 'lfi', 'rfi', 'jwt', 'secret_tokens', 'open_database', 'server_side_js_injection', 'ssti', 'stored_xss'],
